import { formatRentFrequency, formatCurrencyGBP } from './format.mjs';
...
export function calculateWeeklyRent(priceValue, rentFrequency) {
...
export function calculateMonthlyRent(priceValue, rentFrequency) {
  const amount = toNumber(priceValue);
  if (amount == null) {
    return null;
  }
...
export function formatDepositDisplay(info, { fallback = 'Please enquire' } = {}) {
  if (!info) {
...
export function resolveAvailabilityDate(value) {
  if (value == null) {
    return null;
  }
...
export function formatAvailabilityDate(value, { fallback = null } = {}) {
