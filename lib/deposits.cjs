...
function resolveExplicitDepositAmount(value) {
  const numeric = toNumber(value);
  if (numeric == null) {
    return null;
  }
  const rounded = roundCurrency(numeric);
  return rounded != null ? rounded : null;
}
...
function normalizeDeposit(
  ...
  if (typeof deposit === 'number' || (typeof deposit === 'string' && deposit.trim())) {
    const explicit = resolveExplicitDepositAmount(deposit);
    if (explicit != null) {
      return {
        amount: explicit,
        fixed: explicit,
  ...
  const normalizedFixed = normalizeFixedAmount(
    deposit.fixed ?? deposit.amount ?? deposit.value,
    priceValue
  );
  const explicitAmount = normalizeFixedAmount(
    deposit.amount ?? deposit.value,
    priceValue
  );
