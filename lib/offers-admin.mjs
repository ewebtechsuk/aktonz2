...
const ANNUAL_FREQUENCY_TOKENS = new Set([
  'pa',
  'perannum',
  'annually',
  'year',
...
export function formatOfferAmount(offer, type) {
  ...
  if (type === 'rent') {
    const normalizedFrequencyToken = normalizeFrequencyToken(offer?.frequency);
    const baseFrequencyLabel = formatOfferFrequencyLabel(offer?.frequency);
    const frequencyLabel =
      baseFrequencyLabel ||
      (ANNUAL_FREQUENCY_TOKENS.has(normalizedFrequencyToken) ? 'Per annum' : '');
