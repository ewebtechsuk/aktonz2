...
function collectListingImages(listing) {
  ...
  const collections = [listing.images, listing.gallery, listing.photos, listing.propertyImages];
  ...
  const extraKeys = ['imageUrl', 'thumbnailUrl', 'primaryImage', 'mainImage'];
  ...
  const normalized = normalizeImages(Array.from(urls).map((url) => ({ url })));
...
function collectListingMedia(listing) {
  if (!listing || typeof listing !== 'object') {
    return [];
  }
  ...
  const mediaEntries = Array.isArray(listing.media) ? listing.media : [];
  mediaEntries.forEach((entry) => {
    ...
    if (entry && typeof entry === 'object') {
      push(entry.url || entry.href || entry.value);
    }
  });

  extractMedia(listing).forEach(push);
...
function mapListingForAdmin(listing) {
  ...
  const media = collectListingMedia(listing);
  ...
  return {
    ...
    searchIndex,
    raw: listing,
    apexRaw: cloneDeep(listing),
    apexFields: flattenApexFields(listing),
  };
}
...
function applyListingOverride(listing, override) {
  ...
  const next = {
    ...
    apexFields: cloneApexFieldEntries(listing.apexFields),
    apexRaw: listing.apexRaw ? cloneDeep(listing.apexRaw) : null,
  };
  ...
  if (Object.prototype.hasOwnProperty.call(override, 'images')) {
    const images = Array.isArray(override.images) ? override.images.filter(Boolean) : [];
    next.images = [...images];

    if (images.length > 0) {
      const updatedAt = override.updatedAt || next.image?.updatedAt || null;
      next.image = {
        url: images[0],
        thumbnail: images[0],
        updatedAt,
  ...
  if (Object.prototype.hasOwnProperty.call(override, 'media')) {
    next.media = Array.isArray(override.media) ? override.media.filter(Boolean) : [];
  }
...
export function serializeListing(listing, { includeRaw = false, includeApexFields = includeRaw } = {}) {
  if (!listing) {
    return null;
  }

  const { raw, searchIndex, apexRaw, apexFields, ...rest } = listing;
  return {
    ...rest,
    ...(includeApexFields ? { apexFields: cloneApexFieldEntries(apexFields) } : {}),
    ...(includeRaw ? { apexRaw: apexRaw ? cloneDeep(apexRaw) : null } : {}),
  };
}

export function serializeListings(listings, options) {
  return listings.map((listing) => serializeListing(listing, options)).filter(Boolean);
}
...
function sanitizeUrlEntriesInput(value, { typeLabel = 'media item' } = {}) {
  const errors = [];
  const list = Array.isArray(value) ? value : value == null ? [] : [value];
  const sanitized = [];

  list.forEach((entry, index) => {
    const source = typeof entry === 'string' ? entry : entry?.url || entry?.href || '';
    ...
    if (!/^https?:\/\//i.test(raw)) {
      errors.push(`Enter a valid ${typeLabel} URL for item ${index + 1}.`);
      return;
    }
...
function sanitizeListingUpdateInput(updates = {}) {
  ...
  if (Object.prototype.hasOwnProperty.call(input, 'images')) {
    const { values, errors: imageErrors } = sanitizeUrlEntriesInput(input.images, { typeLabel: 'image' });
    if (imageErrors.length > 0) {
      errors.push(...imageErrors);
    }

    override.images = values;
  }

  if (Object.prototype.hasOwnProperty.call(input, 'media')) {
    const { values, errors: mediaErrors } = sanitizeUrlEntriesInput(input.media, { typeLabel: 'media' });
    if (mediaErrors.length > 0) {
      errors.push(...mediaErrors);
    }

    override.media = values;
  }
