...
export function extractMedia(listing) {
  ...
  if (Array.isArray(gallery)) {
    gallery.forEach((item) => {
      const url =
        typeof item === 'string'
          ? item
          : item?.url || item?.href || (typeof item?.value === 'string' ? item.value : null);
      if (!url) return;
  ...
  metadata.forEach((entry) => {
    ...
    const descriptorFields = [entry.type, entry.label, entry.name, entry.title];
    const descriptorText = descriptorFields
      .filter((field) => typeof field === 'string' && field.trim())
      .map((field) => field.toLowerCase());
    const descriptorMatch = descriptorText.some((text) =>
      /tour|video|walkthrough|virtual|360/.test(text)
    );
    if (
      descriptorMatch ||
      lowerValue.includes('matterport.com') ||
      lowerValue.includes('youtube.com') ||
      lowerValue.includes('youtu.be') ||
      lowerValue.includes('vimeo.com') ||
      /\.(mp4|webm|ogg)$/i.test(lowerValue)
    ) {
      push(value);
    }
  });
