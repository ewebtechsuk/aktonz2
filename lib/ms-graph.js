...
const DEFAULT_ALLOWED_UPNS = ["info@aktonz.com"];
...
function resolveBundledClientId() {
...
function determineDefaultTenantId() {
...
const ALLOWED_UPNS = parseAllowedUpns();
const ALLOWED_UPN = ALLOWED_UPNS[0] ?? DEFAULT_ALLOWED_UPNS[0];

const NORMALISED_ALLOWED_UPNS = new Set(ALLOWED_UPNS.map((entry) => entry.toLowerCase()));

function isUpnAllowed(upn) {
  if (!upn) {
    return false;
  }

  return NORMALISED_ALLOWED_UPNS.has(String(upn).trim().toLowerCase());
}
