...
function applyOverridesToContact(contact, override) {
  ...
  if ('stage' in override && override.stage && STAGE_METADATA[override.stage]) {
    ...
    updated.engagementScore = stageMeta.score;
  }
  ...
  if ('generatedNotes' in override) {
    updated.generatedNotes = override.generatedNotes || null;
  }
...
function sanitiseContactUpdateInput(updates) {
  ...
  if ('generatedNotes' in input) {
    const value = sanitizeString(input.generatedNotes);
    override.generatedNotes = value || null;
  }
