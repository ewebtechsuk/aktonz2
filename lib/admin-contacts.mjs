...
const contactsSource = loadJson('../data/apex27-contacts.json', import.meta.url, {});
const agentsSource = loadJson('../data/agents.json', import.meta.url, []);
...
const FALLBACK_CONTACTS = Array.isArray(contactsSource?.contacts) ? contactsSource.contacts : [];
const FALLBACK_GENERATED_AT = contactsSource?.generatedAt || null;
...
const structuredCloneFn =
  typeof globalThis.structuredClone === 'function' ? globalThis.structuredClone.bind(globalThis) : null;
...
function cloneDeep(value, seen = new WeakMap()) {
  ...
  if (structuredCloneFn) {
    try {
      ...
      // Fall back to manual cloning below if structuredClone fails.
    }
  }
  ...
  try {
    ...
    return cloneDeepFallback(value, seen);
  }
}

function cloneDeepFallback(value, seen) {
  if (value == null) {
    return value;
  }
  ...
  if (Array.isArray(value)) {
    ...
    value.forEach((item) => {
      clone.push(cloneDeepFallback(item, seen));
    });
    return clone;
  }
  ...
  if (value instanceof Map) {
    ...
    for (const [key, entry] of value.entries()) {
      clone.push([cloneDeepFallback(key, seen), cloneDeepFallback(entry, seen)]);
    }
    return clone;
  }
  ...
  if (value instanceof Set) {
    ...
    for (const entry of value.values()) {
      clone.push(cloneDeepFallback(entry, seen));
    }
    return clone;
  }

  if (ArrayBuffer.isView(value)) {
    const clone = Array.from(value, (item) => cloneDeepFallback(item, seen));
    seen.set(value, clone);
    return clone;
  }
  ...
  for (const [key, nested] of Object.entries(value)) {
    ...
    const clonedValue = cloneDeepFallback(nested, seen);
