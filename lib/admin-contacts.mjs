...
const {
  ...
  const fallbackContactsSource = loadJson('data/apex27-contacts.json', {
    contacts: [],
    generatedAt: null,
  });

  return {
    FALLBACK_CONTACTS: Array.isArray(fallbackContactsSource?.contacts)
      ? fallbackContactsSource.contacts
      : [],
    FALLBACK_GENERATED_AT: fallbackContactsSource?.generatedAt || null,
    AGENT_MAP: new Map(
      (Array.isArray(agentsSource) ? agentsSource : []).map((agent) => [
        String(agent.id),
        {
          id: String(agent.id),
