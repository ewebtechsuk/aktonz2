...
const explicitLiveFetchFlag = parseEnvBoolean(process.env.SCRAYE_ALLOW_LIVE_FETCH);
const defaultLiveFetchDisallowed = [
  parseEnvBoolean(process.env.NEXT_EXPORT) === true,
  parseEnvBoolean(process.env.CI) === true,
  parseEnvBoolean(process.env.GITHUB_ACTIONS) === true,
].some(Boolean);

const SCRAYE_LIVE_FETCH_ALLOWED =
  explicitLiveFetchFlag ?? !defaultLiveFetchDisallowed;

const scrayeWarningsIssued = new Set();
function warnOnce(key, message) {
  if (scrayeWarningsIssued.has(key)) {
    return;
  }
  scrayeWarningsIssued.add(key);
...
async function enrichScrayeListingsWithDetails(
  ...
  if (!SCRAYE_LIVE_FETCH_ALLOWED) {
    warnOnce(
      'scraye-detail-enrichment-disabled',
      'Skipping Scraye detail enrichment because live fetching is disabled.'
    );
    return listings;
  }
...
export async function fetchScrayeListings({
  ...
  if (!SCRAYE_LIVE_FETCH_ALLOWED) {
    warnOnce(
      'scraye-live-fetch-disabled',
      'Scraye live fetching is disabled; returning cached results only.'
    );
    return [];
  }
...
export async function loadScrayeListingsByType(type) {
  ...
  if (!Array.isArray(listings) || listings.length === 0) {
    if (!SCRAYE_LIVE_FETCH_ALLOWED) {
      warnOnce(
        'scraye-live-fetch-disabled',
        'Scraye live fetching is disabled; falling back to cached listings only.'
      );
      return Array.isArray(listings) ? listings : [];
    }
    try {
