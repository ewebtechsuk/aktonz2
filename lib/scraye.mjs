...
function getHttpAgent() {
  ...
  if (!cachedHttpAgent) {
    cachedHttpAgent = new Agent({
      connect: {
        // Ensure IPv4 is preferred in environments where IPv6 routing is unavailable.
        family: 4,
      },
    });
  }
...
function normalizeListingNode(node, context) {
  ...
  const normalizedTransactionType =
    context.transactionType ?? (node.type === 'SALE' ? 'sale' : 'rent');
  const rawStatus = node.status ?? null;
  const normalizedStatus = (() => {
    if (!rawStatus) {
      return rawStatus;
    }
    const statusUpper = String(rawStatus).trim().toUpperCase();
    if (normalizedTransactionType === 'sale' && statusUpper === 'CANCELLED') {
      return 'AVAILABLE';
    }
    return rawStatus;
  })();
...
export async function fetchScrayeListingById(
  ...
  const loaderPromise = (async () => {
    ...
    try {
      ...
      const detailLocation = Array.isArray(listing.location?.coordinates)
        ? listing.location
        : null;
      const normalized = normalizeListingNode(
        {
          ...
          location:
            ...
            (baseEntry?.latitude
              ? {
                  type: 'Point',
                  coordinates: [baseEntry.longitude, baseEntry.latitude],
                }
              : undefined),
        },
        baseContext
