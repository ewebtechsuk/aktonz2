function formatRentFrequency(freq) {
  if (!freq) return '';
...
function formatPricePrefix(prefix) {
  if (!prefix) return '';
...
function formatPriceGBP(value, { isSale = false } = {}) {
  const amount = normalizePriceAmount(value);
  if (amount == null) return '';
  const formattedNumber = isSale ? amount.toLocaleString('en-GB') : String(amount);
  return `£${formattedNumber}`;
}

function formatCurrencyGBP(value, options = {}) {
