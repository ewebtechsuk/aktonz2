...
function resolveOfferTimestamp(offer) {
  if (!offer) {
    return null;
  }

  return getAdminTimestamp(offer.updatedAt || offer.date);
}
...
export default function AdminOffersPage() {
  ...
  const loadOffers = useCallback(async () => {
    ...
    try {
      ...
      entries.sort((a, b) => {
        const aTimestamp = resolveOfferTimestamp(a) || 0;
        const bTimestamp = resolveOfferTimestamp(b) || 0;
